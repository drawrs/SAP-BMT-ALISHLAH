...
if (! function_exists('getAlamat')) {
     ...
     * @return \Illuminate\View\View|\Illuminate\Contracts\View\Factory
     */
    function getAlamat($ambil = NULL, $alamat = "######")
    {
        $get = explode('#', $alamat);
            switch ($ambil) {
                case 'blk':
...
if (! function_exists('autoTab')) {
    /**
     * Buat radio button
     * jadi kalo nilainya sama bakal langsung checked
     */
    function autoTab($y = "1") // defaultnya sngaja dibikin false
    {
        $x = (is_null(Request::input('v')) ? "tabInfo": Request::input('v'));
        if ($x == $y) {
            $result = "active";
        } else {
            $result = "";

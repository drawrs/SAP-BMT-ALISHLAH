    ...
    @endphp
        @forelse($jaminan as $jmn)
            ...
            <tr>
                <td>
                    {{ $no++ }}
                    <a href="{{ url('hapus-jaminan?id='. $jmn->id) }}" onclick="return confirm('Hapus?')" class="btn btn-danger btn-xs pull-right"><i class="fa fa-minus fa-xs"></i></a>
                </td>
                <td>{{ $jmn->dokumen }}</td>
                <td>{{ $jmn->bentuk }}</td>
                <td>{{ $jmn->kuantitas }}</td>
...
<table class="table table-bordered">
    ...
    <tbody>
        ...
        <tr>
            <td>
                {{ $no++ }}
                <a href="{{ url('hapus-data-pemilik?id='. $pemilik->id) }}" onclick="return confirm('Hapus?')" class="btn btn-danger btn-xs pull-right"><i class="fa fa-minus fa-xs"></i></a>
            </td>
            <td>{{ $pemilik->nama }}</td>
    ...
    <table class="table table-bordered">
        ...
        <tbody>
            <tr>
                ...
                <td rowspan="9">
                    @foreach($rek_pers as $rek)
                        <b>{{ $rek->user->name }} ( {{ $rek->user->detail->jabatan  }} )</b> : {{ $rek->pesan }}
                        <a href="{{ url('hapus-rek-pers?id=' . $rek->id) }}" onclick="return confirm('Hapus ?')" class="btn btn-danger btn-xs pull-right"><i class="fa fa-minus"></i></a>
                        <hr>
                    @endforeach
                </td>
                <td rowspan="9"></td>
...
<div class="modal fade" id="addRekomen" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="wrapper" style="margin-top: 0">
          <form action="{{ url('tambah-rek-pers') }}" method="POST" class="form-horizontal">
